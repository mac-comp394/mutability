module Msg
  ...
  class Add
    def apply_to(model)
      ...
      new_entries = model.entries.dup  # Need to make a non reference copy
  ...
  class Check
    ...
    def apply_to(model)
      new_entries = model.entries.map do |entry|
        if entry.id == id
          Entry.new(id: entry.id, description: entry.description, completed: is_completed)
        else
          entry
        end
      end
